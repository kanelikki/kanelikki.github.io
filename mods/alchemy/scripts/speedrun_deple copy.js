...
/*
const depthMap = all.map(id => {
    obj.length
})
*/
const depthMap = new Map(); //id, depth
const getChildrenLength = function(id, created) {
    if(created.has(id)) return 1;
    if(depthMap.has(id)) return depthMap.get(id);
    const obj = stat.getObject(id);
    created.add(id);
    let len = created.size;
    if(obj.children.length === 0) {
        depthMap.set(id, len);
        return len;
    }
    for(const c of obj.children) {
        if(depthMap.has(c)) len += depthMap.get(c);
        else len += getChildrenLength(c, created);
    }
    depthMap.set(id, len);
    return len;
}
const primeIds = workspace.__vue__.$store.getters.prime;
primeIds.forEach(id => getChildrenLength(id, new Set()));
...
function isFoundExhaust(workspace, currentLib, lib) {
    ...
    function setPriority(val) {
        ...
        //if(highPriorities.has(val)) return -9999;
        for(const id of ids) {
